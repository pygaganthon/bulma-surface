defmodule Bulma.Components.Pagination do
  ...
  def render(assigns) do
    ~H"""
      <nav class={{ "pagination", @class, "is-centered": @centered, "is-right": @right, "is-rounded": @rounded, "is-#{@size}": @size}} role="navigation" aria-label="pagination">
        <slot/>
        <ul class="pagination-list">
          <li :for.with_index={{ {item, index} <- @list }}>
            <a
              ...
              :on-click={{ item.click }} phx-value-data={{ item.data }}
              >
              <slot name="list" index={{ index }} :if={{ !item.label }}></slot>
              {{ item.label }}
            </a>
