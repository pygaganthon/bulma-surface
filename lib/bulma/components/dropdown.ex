defmodule Bulma.Components.Dropdown do
  ...
  def render(assigns) do
    ~H"""
      <div class={{ "dropdown", @class, "is-active": @active, "is-hoverable": @hoverable, "is-#{@alignment}": @alignment, "is-up": @up, "is-#{@color}": @color }}>
        <div class="dropdown-trigger">
          <Bulma.Elements.Button color={{ @color }} icon={{ @icon }} click={{ if @hoverable do nil else "toggle-dropdown" end }} >
            <span>{{ @label }}</span>
            <Bulma.Elements.Icon icon={{ @trigger_icon || if @up do "angle-up" else "angle-down" end}} size="small"/>
