defmodule Bulma.Components.Panel.AnchorBlock do
  ...
  def render(assigns) do
    ~H"""
      <a class={{ "panel-block", "is-active": @active}} href="{{@url}}" :on-click={{ @click }}>
        <Bulma.Elements.Icon class="panel-icon" icon={{ @icon }} size={{ @icon_size }} container_size={{ @icon_container_size }} :if={{ @icon }} />
        <slot>{{ @label }}</slot>
      </a>
    """
  end
